...
const handlers = [
  rest.get("api/v1/clusterState", (_, res, ctx) => {
    return res(
      ctx.status(200),
      ctx.json(mockResponses.ClusterState)
    );
  }),
  rest.get("api/v1/task/status", (_, res, ctx) => {
    return res(
      ctx.status(200),
      ctx.json(mockResponses.TaskStatus)
    );
  }),
  rest.get("api/v1/keys/open/summary", (_, res, ctx) => {
    return res(
      ctx.status(200),
      ctx.json(mockResponses.OpenKeys)
    );
  }),
  rest.get("api/v1/keys/deletePending/summary", (_, res, ctx) => {
    return res(
      ctx.status(200),
