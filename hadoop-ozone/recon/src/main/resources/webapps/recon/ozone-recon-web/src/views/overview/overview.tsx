...
import { CheckCircleFilled, ClusterOutlined, ContainerFilled, DatabaseFilled, DeleteOutlined, DeploymentUnitOutlined, ExclamationCircleFilled, FileTextFilled, FileTextOutlined, FolderOpenOutlined, InboxOutlined } from '@ant-design/icons';
...
const Overview = (props: Record<string, object>) => {
  const [overviewState, setOverviewState] = useState<IOverviewState>({
    loading: false,
    datanodes: '',
    pipelines: 0,
    storageReport: {
  ...
  const autoReload = new AutoReloadHelper(_loadData);
  ...
  const { loading, datanodes, pipelines, storageReport, containers, volumes, buckets, openSummarytotalUnrepSize, openSummarytotalRepSize, openSummarytotalOpenKeys,
    ...
    keys, missingContainersCount, lastRefreshed, lastUpdatedOMDBDelta, lastUpdatedOMDBFull, omStatus, openContainers, deletedContainers } = overviewState;
    ...
    <div className='overview-content'>
      <div className='page-header'>
        ...
        <AutoReloadPanel isLoading={loading} lastRefreshed={lastRefreshed}
          ...
          togglePolling={autoReload.handleAutoReloadToggle} onReload={_loadData} omSyncLoad={omSyncData} omStatus={omStatus} />
      </div>
      <Row gutter={[25, 25]}>
        <Col xs={24} sm={18} md={12} lg={12} xl={6}>
          <OverviewCard
            hoverable loading={loading} title='Datanodes'
            data={datanodesElement} icon={<ClusterOutlined/>}
            linkToUrl='/Datanodes' />
        </Col>
        <Col xs={24} sm={18} md={12} lg={12} xl={6}>
          <OverviewCard
            hoverable loading={loading} title='Pipelines'
            data={pipelines.toString()} icon={<DeploymentUnitOutlined/>}
            linkToUrl='/Pipelines' />
        </Col>
        <Col xs={24} sm={18} md={12} lg={12} xl={6}>
          <OverviewCard
      ...
      <Row gutter={[25, 25]}>
        <Col xs={24} sm={18} md={12} lg={12} xl={6}>
          <OverviewCard loading={loading} title='Volumes' data={volumes.toString()} icon={<InboxOutlined/>} linkToUrl={duLink} />
        </Col>
        <Col xs={24} sm={18} md={12} lg={12} xl={6}>
          <OverviewCard loading={loading} title='Buckets' data={buckets.toString()} icon={<FolderOpenOutlined/>} linkToUrl='/Buckets' />
        </Col>
        <Col xs={24} sm={18} md={12} lg={12} xl={6}>
          <OverviewCard loading={loading} title='Keys' data={keys.toString()} icon={<FileTextOutlined/>} />
        </Col>
        <Col xs={24} sm={18} md={12} lg={12} xl={6}>
          <OverviewCard loading={loading} title='Deleted Containers' data={deletedContainers.toString()} icon={<DeleteOutlined/>} />
        </Col>
        <Col xs={24} sm={18} md={12} lg={12} xl={6}>
          <OverviewCard loading={loading} title='Pending Key Deletions' data={keysPendingDeletion.toString()} icon={<DeleteOutlined/>} />
        </Col>
        <Col xs={24} sm={18} md={12} lg={12} xl={6} className='summary-font'>
          <OverviewCard loading={loading} title='Open Keys Summary' data={openSummaryData} icon={<FileTextOutlined/>} />
        </Col>
        <Col xs={24} sm={18} md={12} lg={12} xl={6} className='summary-font'>
          <OverviewCard loading={loading} title='Pending Deleted Keys Summary' data={deletePendingSummaryData} icon={<DeleteOutlined/>} />
        </Col>
      </Row>
    </div>
  );
